...
$codigo_persona = "";
$sueldo_fijo=isset($_POST["sueldo_fijo"])? limpiarCadena($_POST["sueldo_fijo"]):"";
$password=isset($_POST["codigo_persona"])? limpiarCadena($_POST["codigo_persona"]):"";
...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		...
		if (empty($idusuario)) {
			...
			$rspta=$usuario->insertar($nombre,$apellidos,$login,$iddepartamento,$idtipousuario,$email,$clavehash,$imagen,$usuariocreado,$codigo_persona, $sueldo_fijo, $idempresa);
			echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar todos los datos del usuario";
		}
		else {
			$rspta=$usuario->editar($idusuario,$nombre,$apellidos,$login,$iddepartamento,$idtipousuario,$email,$imagen,$usuariocreado,$codigo_persona, $sueldo_fijo, $idempresa);
	...
	case 'listar':
		$rspta=$usuario->listar($_SESSION["idempresa"]);
		//declaramos un array
		$data=Array();
		...
		while ($reg=$rspta->fetch_object()) {
			$data[]=array(
				...
				.' '.'<button class="btn btn-default btn-xs" onclick="eliminar('.$reg->idusuario.')"><i class="fa fa-trash"></i></button>'
	...
	case 'verificar':
		...
		$rspta=$usuario->verificar($logina, $clavehash, $ruca);
		...
		if (isset($fetch))
		{
			...
			$_SESSION['sueldo_fijo']=$fetch->sueldo_fijo;
			...
			$_SESSION['EMPRESA']=$fetch->razonsocial;
